...
'''Minimizes a 2d function. exact line search for alpha. is getting stuck before minimizing'''
...
def alphak(xa, da,k): # going to try exact line search sonce function is quadratic
    x1 = xa[0][0]
    x2 = xa[1][0]
    d1 = da[0][0]
    d2 = da[1][0]
    num = (-1 * d1 * x1) + (d1 * x2) + (d2 * x1) - (4 * d2 * x2)
    den = (d2 ** 2) - (2 * d1 * d2) + (4 * (d2 ** 2))
    return num / den
...
with open('finalp6.csv', mode='w') as data_file:
    data_writer = csv.writer(data_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    k = 1
    while True and k < 10000:
        d_raw = m.scalar_mult(1, m.matrix_mult(B[k], grad_f(x[k])))
        d_norm = m.scalar_mult(1 / v.euc_norm(d_raw), d_raw)
        d.append(d_norm)

...
# try to fix data display table for selected pokemon
...
app.layout = html.Div([    
    ...
    html.Div([
        ...
        html.Div([
            ...
            dash_table.DataTable(id='poke-info')
        ], style={'display': 'inline-block','width': '25%', 'float': 'top right', 'overflow': 'scroll', 'max-height': '300px', 'padding-bottom': '50px'})
    ])
])
...
def update_selected_poke(selected_poke):
    ...
    columns = [{"name": 'Attributes', "id": '1'}, {"name": 'Pokemon', "id": '0'}]
    print(columns)
